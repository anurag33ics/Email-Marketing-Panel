                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12">
                                ...
                                <div class="card-body pt-5">
                                    <div class="row">
                                        ...
                                        if($_SESSION['role']=="manager")
                                            $stats1 = $con->query("SELECT distinct(subject) FROM sendemaildata WHERE session_id='".$_SESSION['id']."'");
                                        else
                                          $stats1 = $con->query("SELECT distinct(subject) FROM sendemaildata");
                                        ...
                                        <div class="col-md-6" style="text-align: end;">
                                            <a href="manual_template.php" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i>&nbsp; Compose Email</a>
                            ...
                            <button onclick='exportToExcel()'class="btn btn-primary btn-sm"><i class="fa fa-download" aria-hidden="true"></i>Export to Excel</button>
                            <div class="table-new">
                                    ...
                                    <table id="example1" class="table table-bordered zero-configuration">
                                        ...
                                        <tbody>
                                            <?php
                                             $id=$_SESSION['login_id'];
                                            if($_SESSION['role']=='admin'){
                                            $query=mysqli_query($con,"select * from sendemaildata WHERE email_type='manual' order by id desc");
                                            }
                                            else{
                                                 $query=mysqli_query($con,"select * from sendemaildata where session_id='$id' AND email_type='manual' order by id desc ");
                                            }
                                            $cnt=1;
                                            ...
                                            <tr>
                                                ...
                                                <!--<td>-->
                                                    <?php //echo date("d-m-Y H:i:s",strtotime($row['email_open_time']//))?>
                                                    <!--</td>-->
                                                <?php //} ?>
...
function exportToExcel() {
  const wb = XLSX.utils.table_to_book(document.getElementById('example1'), {sheet: "Email History"});
...
url:'ajax.php?action=updateStatusEmail',
 ...
 success:function(resp){
 if(resp == 1){
     $("#sucess").text("Successfully updated");
setTimeout(function(){
        window.location.reload();
    },2000)
 }else{
     $("#sucess").text("Something went wrong");
     setTimeout(function(){
        window.location.reload();
    },2000)
 }
